...
**CoralDesk** 将 ZeroClaw 运行时包装进一个基于 Flutter 构建的、跨平台的原生桌面应用。ZeroClaw 的 Rust 库通过 [flutter_rust_bridge](https://github.com/fzyzcjy/flutter_rust_bridge)（FFI）**直接嵌入进程内**——无需 HTTP 服务进程，无需子进程，无需守护程序。你可以获得 ZeroClaw Agent 引擎的完整能力，同时拥有流畅、原生的图形界面。
...
### 🤖 Agent 与工作空间
- 配置委托/子 Agent，支持自定义角色和提供商覆盖
- 管理多个 Agent 工作空间，各自独立的根目录路径

### ⚙️ 配置
- 自主级别调节（监督模式 ↔ 完全自主模式）
- 逐工具权限控制（允许 / 拒绝 / 每次确认）
- Agent 循环参数（最大迭代次数、工具调用次数上限）
- 成本预算与用量上限控制
...
```bash
git clone https://github.com/zibo-chen/coraldesk.git
cd coraldesk
```
...
<p align="center">
  <img src="docs/screenshots/Preferences.png" alt="系统偏好" width="70%" />
</p>
...
├── constants.dart         # 全局常量
├── models/                # Freezed 数据模型（ChatMessage、ChatSession、Project…）
├── providers/             # Riverpod 状态提供者（chat、sessions、theme、projects…）
├── services/              # UpdateService、MCP 测试客户端等
├── theme/                 # 亮色/暗色 AppTheme
├── views/
│   ├── shell/             # 根布局壳（侧边栏 + 面板）
│   ├── sidebar/           # 可折叠导航侧边栏
...
zeroclaw/                  # ZeroClaw Rust crate（git submodule / 本地路径依赖）
```
